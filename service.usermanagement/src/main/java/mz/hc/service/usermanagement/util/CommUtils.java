package mz.hc.service.usermanagement.util;
...
import mz.hc.service.usermanagement.dto.ApiResultCode;


public class CommUtils {

    // email pattern
    private static Pattern emailPattern = Pattern.compile("^([0-9a-zA-Z]?[-_\\.]?)*@[0-9a-zA-Z]([-_\\.]?[0-9a-zA-Z])*\\.[a-zA-Z]{2,4}$");
    ...
    public static int getTotalPage(int totalCount, int size) {
        if (totalCount < 1 || size < 1) return 0;
    ...
    public static boolean isDate(String value) {
        value = value.replaceAll("[^0-9]","");
        if (value.length() < 8) return false;
        if (value.length() > 8) return isDateTime(value);
        try {
