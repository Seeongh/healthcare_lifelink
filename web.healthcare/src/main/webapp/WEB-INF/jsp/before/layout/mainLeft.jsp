...
<section class="lnb_wrap">
		...
		<nav class="lnb">
		    ...
		    <%-- <ul class="lnb_list" id="lnbMenu">
		    	<li class="lnb_list__item" id="leftMenu12100">
		 			<a class="lnb_list__link lnb_list__link<c:if test="${fn:split(uri,'/')[1] eq 'mypage'}">--on</c:if>" href="/userInfo/mypage">내정보</a>
			 	</li>
			</ul>
		  <c:if test="${!empty acToken }">
			    <ul class="lnb_list" id="lnbMenu">
			    	<li class="lnb_list__item" id="leftMenu12100">
			 			<a class="lnb_list__link lnb_list__link" href="<c:url value='/user/logout'/>">로그아웃</a>
				 	</li>
				</ul>
		  </c:if> --%>
	</nav>
